...
next_chars = []
for line in lines:
    list = data_reader.get_corpus_list(line, SEQUENCE_LENGTH)
    for sentence, lable in list:
        sentences.append(sentence)
        next_chars.append(lable)
...
for i, sentence in enumerate(sentences):
    start_index = SEQUENCE_LENGTH - len(sentence)
    for t, char in enumerate(sentence):
        if char in char_indices:
            X[i, start_index + t, char_indices[char]] = 1
